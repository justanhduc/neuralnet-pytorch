...
torch::Tensor pointcloud_to_voxel_cpp(torch::Tensor pc, int voxel_size, float grid_size, bool filter_outlier) {
    ...
    auto batch_indices = torch::arange(b).to(pc.device().type()).to(indices.dtype());
    ...
    if (filter_outlier) {
        valid = at::all(at::__and__(at::ge(pc, -half_size), at::le(pc, half_size)), 2);
        valid = valid.flatten();
    ...
    for (int k=0; k < 2; ++k) {
        for (int j=0; j < 2; ++j) {
            for (int i=0; i < 2; ++i) {
                auto updates = rr[k].slice(2, 0, 1) * rr[j].slice(2, 1, 2) * rr[i].slice(2, 2, 3);

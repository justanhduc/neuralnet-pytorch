...
__global__ void NmDistanceKernel(int b, int n, const scalar_t* __restrict__ xyz, int m, const scalar_t* __restrict__ xyz2,
...
std::vector<torch::Tensor> chamfer_cuda_forward(at::Tensor xyz1, at::Tensor xyz2) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND(at::ScalarType::Half, xyz1.scalar_type(), "chamfer_cuda_forward", ([&] {
...
template<typename scalar_t>
__global__ void NmDistanceGradKernel(int b, int n, const scalar_t* __restrict__ xyz1, int m, const scalar_t* __restrict__ xyz2,
                                     const scalar_t* __restrict__ grad_dist1, const scalar_t* __restrict__ idx,
                                     scalar_t* __restrict__ grad_xyz1, scalar_t* __restrict__ grad_xyz2) {
	for (int i=blockIdx.x;i<b;i+=gridDim.x){
		for (int j=threadIdx.x+blockIdx.y*blockDim.x;j<n;j+=blockDim.x*gridDim.y){
...
std::vector<torch::Tensor> chamfer_cuda_backward(at::Tensor xyz1, at::Tensor xyz2, at::Tensor graddist1,
    ...
    AT_DISPATCH_FLOATING_TYPES_AND(at::ScalarType::Half, xyz1.scalar_type(), "chamfer_backward_cuda", ([&] {
        NmDistanceGradKernel<scalar_t><<<dim3(1,16,1),256>>>(
            ...
            m,
            xyz2.data<scalar_t>(),
            graddist1.data<scalar_t>(),
            idx1.data<scalar_t>(),
            gradxyz1.data<scalar_t>(),
            gradxyz2.data<scalar_t>());
        NmDistanceGradKernel<scalar_t><<<dim3(1,16,1),256>>>(
            batch_size,
            m,
            xyz2.data<scalar_t>(),
